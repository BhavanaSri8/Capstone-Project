...
class SubscriptionControllerTest {
    ...
    void getAllSubscriptions_returnsOk() throws Exception {
        ...
        when(subscriptionService.getAllSubscriptions()).thenReturn(List.of(first, second));
    ...
    void getSubscriptionsByUser_returnsOk() throws Exception {
        ...
        when(subscriptionService.getSubscriptionsByUser(7L)).thenReturn(List.of(subscription));

        mockMvc.perform(get("/api/subscriptions/user/7"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].subscriptionId").value(10));
    }
