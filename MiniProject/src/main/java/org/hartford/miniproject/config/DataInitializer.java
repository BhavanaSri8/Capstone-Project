...
public class DataInitializer implements CommandLineRunner {
    ...
    private void createDefaultUser(String email, String roleName, long userId) {
        if (userRepository.findByEmail(email).isEmpty()) {
            Role role = roleRepository.findByRoleName(roleName).orElse(null);
            if (role == null) {
                System.err.println("Warning: Role not found: " + roleName + ". Skipping default user creation for " + email + " (this is expected if RoleRepository is mocked).");
                return;
            }
            User user = new User();
            ...
            user.setEmail(email);
            user.setPassword(passwordEncoder.encode("Bhavana@12"));
            user.setRole(role);
            user.setAge(25);
